STM32F103 High-Performance SPI Submodule 🚀
...
⚙️ Main Features
Simple Configuration: Based on clear structs and strongly typed enums (e.g., SPI_Polarity, SPI_BaudRate, SPI_DataOrder) to prevent setup errors.
...
Integrated DMA Support: Dedicated APIs to leverage DMA channels (SPI1_DMA_Init, SPI1_DMA_Transmit, SPI1_DMA_Receive) for efficient asynchronous transfers.
...
Add this repository as a Git submodule inside your STM32 project (ideally cloning it into a convenient path like Core/Submodules/SPI).

In your compiler/IDE settings (e.g., STM32CubeIDE), go to Include paths and add the path to the Inc folder of this submodule, so the compiler can resolve the #include directives.

Make sure the source .cpp files are included in your build paths (Source location).

Include "SPIConfig.hpp" or "SPI.hpp" in your scripts and start writing high-performance code!
